...
const transformImage = async (img, brushSize, colorCount, anchor = null) => {
  const beforeCanvas = document.createElement('canvas');
  const beforeCtx = beforeCanvas.getContext('2d', { willReadFrequently: true });
  fitCanvasToImage(beforeCanvas, beforeCtx, img);

  const size = Math.max(1, Math.round(Number(brushSize) || 1));
  const step = Math.max(1, Math.round(255 / (Math.max(2, Number(colorCount) || 16) - 1)));
  const x = anchor?.x ?? Math.floor(beforeCanvas.width / 2);
  const y = anchor?.y ?? Math.floor(beforeCanvas.height / 2);
  ...
  for (let outY = 0, gy = yStart; gy < beforeCanvas.height; outY++, gy += size) {
    for (let outX = 0, gx = xStart; gx < beforeCanvas.width; outX++, gx += size) {
      ...
      out.data[idx] = quantize(src.data[sample], step);
      out.data[idx + 1] = quantize(src.data[sample + 1], step);
      out.data[idx + 2] = quantize(src.data[sample + 2], step);
      out.data[idx + 3] = src.data[sample + 3];
    }
  }
...
const createFrontmatterText = ({ sourcePath, outputName, brushSize, colorCount, result }) => `---
source: "${sourcePath.replace(/"/g, '\\"')}"
after_image: "${outputName.replace(/"/g, '\\"')}"
brush_size: ${brushSize}
color_count: ${colorCount}
anchor_x: ${result.anchor.x}
anchor_y: ${result.anchor.y}
source_width: ${result.beforeWidth}
source_height: ${result.beforeHeight}
after_width: ${result.outputWidth}
after_height: ${result.outputHeight}
...
const I18N = {
  en: {
    ...
    batchTitle: 'Batch Folder Convert',
    batchAddImages: 'Add Images',
    batchRun: 'Convert All',
    batchEmpty: 'Connect a folder or add images to prepare a batch.',
    batchLoaded: (count) => `Loaded ${count} image(s) for batch conversion.`,
    batchAdded: (count) => `Added ${count} image(s) to the batch.`,
    batchDone: (count) => `Converted ${count} image(s). Results are available in Completed.`,
    batchOutputCancelled: 'Output folder selection was cancelled.',
    gallery: 'Gallery',
    completed: 'Completed',
  ...
  ko: {
    ...
    batchConvert: '일괄 변환',
    batchTitle: '폴더 일괄 변환',
    batchAddImages: '이미지 추가',
    batchRun: '전체 변환',
    batchEmpty: '폴더를 연결하거나 이미지를 추가해 일괄 변환을 준비하세요.',
    batchLoaded: (count) => `${count}개 이미지를 일괄 변환 목록에 불러왔습니다.`,
    batchAdded: (count) => `${count}개 이미지를 일괄 변환 목록에 추가했습니다.`,
    batchDone: (count) => `${count}개 이미지를 변환했습니다. 완성 탭에서 확인할 수 있습니다.`,
    batchOutputCancelled: '저장 폴더 선택이 취소되었습니다.',
    gallery: '갤러리',
    completed: '완성',
    beforeAfter: 'Before / After',
    before: 'Before',
  ...
  ja: {
    ...
    batchConvert: '一括変換',
    batchTitle: 'フォルダー一括変換',
    batchAddImages: '画像追加',
    batchRun: 'すべて変換',
    batchEmpty: 'フォルダー接続または画像追加で一括変換を準備してください。',
    batchLoaded: (count) => `${count}枚の画像を一括変換リストに読み込みました。`,
    batchAdded: (count) => `${count}枚の画像を一括変換リストに追加しました。`,
    batchDone: (count) => `${count}枚の画像を変換しました。完了タブで確認できます。`,
    batchOutputCancelled: '保存先フォルダーの選択がキャンセルされました。',
    gallery: 'ギャラリー',
    completed: '完了',
    beforeAfter: 'Before / After',
    before: 'Before',
  ...
  zh: {
    ...
    batchAddImages: '添加图片',
    batchRun: '全部转换',
    batchEmpty: '请连接文件夹或添加图片来准备批量转换。',
    batchLoaded: (count) => `已加载 ${count} 张图片用于批量转换。`,
    batchAdded: (count) => `已向批量列表添加 ${count} 张图片。`,
    batchDone: (count) => `已转换 ${count} 张图片。可在完成标签中查看。`,
    batchOutputCancelled: '已取消选择输出文件夹。',
    gallery: '图库',
    completed: '完成',
    beforeAfter: 'Before / After',
    before: 'Before',
...
function App() {
  ...
  const [batchBrushSize, setBatchBrushSize] = useState(brushSize);
  const [batchColorCount, setBatchColorCount] = useState(colorCount);
  ...
  const selectedItem = useMemo(
    () => (
      singleItem?.id === selectedId
        ? singleItem
        : items.find((it) => it.id === selectedId) || batchResults.find((it) => it.id === selectedId) || null
    ),
    [items, selectedId, singleItem, batchResults],
  );
  ...
  const readBatchItemsFromDirectory = async (dirHandle) => {
    const list = [];
    let index = 0;
    for await (const entry of dirHandle.values()) {
      if (entry.kind !== 'file') continue;
  ...
  const readBatchItemsFromFiles = async (files) => {
    const list = [];
    let index = 0;
    for (const file of files) {
      if (!IMAGE_TYPES.has(file.type)) continue;
  ...
  const handleOpenBatch = async () => {
    ...
    try {
      ...
      const list = await readBatchItemsFromDirectory(handle);
      setBatchItems(list);
      setBatchBrushSize(brushSize);
      setBatchColorCount(colorCount);
      setBatchOpen(true);
      setStatus(text.batchLoaded(list.length));
    } catch (err) {
      setStatus(text.folderError(err.message));
    }
  };

  const handleAddBatchImages = async (event) => {
    const list = await readBatchItemsFromFiles(Array.from(event.target.files || []));
    setBatchItems((prev) => [...prev, ...list]);
    setStatus(text.batchAdded(list.length));
    event.target.value = '';
  };
  ...
  const handleRunBatch = async () => {
    if (!batchItems.length || !window.showDirectoryPicker) return;

    let outputDir;
    try {
      outputDir = await window.showDirectoryPicker({ mode: 'readwrite' });
    } catch (err) {
      setStatus(text.batchOutputCancelled);
      return;
    }

    const converted = [];
    for (const item of batchItems) {
      const result = await transformImage(item.beforeImage, batchBrushSize, batchColorCount);
      const outputName = makeOutputName(item.name);
      const metaName = outputName.replace(/\.png$/i, '.txt');
      await writeFile(outputDir, outputName, result.blob);
      await writeFile(outputDir, metaName, createFrontmatterText({
        sourcePath: item.sourcePath,
        outputName,
        brushSize: batchBrushSize,
        colorCount: batchColorCount,
        result,
      }));
      converted.push({
        id: `batch-result-${Date.now()}-${converted.length}`,
        name: outputName,
        ...
        saved: true,
        savedThumbDataUrl: createThumbDataUrl(result.afterImage),
        source: 'batch',
      });
    }

    setBatchResults((prev) => [...converted, ...prev]);
    setBatchOpen(false);
    setActiveTab('completed');
    setStatus(text.batchDone(converted.length));
  };
  ...
  const processCurrentImage = async (x, y) => {
    ...
    const result = await transformImage(item.beforeImage, brushSize, colorCount, { x, y });
    afterCanvas.width = result.outputWidth;
    afterCanvas.height = result.outputHeight;
    afterCtx.imageSmoothingEnabled = false;
    afterCtx.drawImage(result.afterImage, 0, 0);
    setCanvasSizes({
      before: { width: beforeCanvas.width, height: beforeCanvas.height },
      after: { width: afterCanvas.width, height: afterCanvas.height },
    });

    setItems((prev) => prev.map((it) => (it.id === item.id ? { ...it, afterBlob: result.blob, afterImage: result.afterImage } : it)));
    if (singleItem?.id === item.id) {
      setSingleItem((prev) => (prev ? { ...prev, afterBlob: result.blob, afterImage: result.afterImage } : prev));
    }
    setUnsaved(true);
  ...
  const completedItems = [...batchResults, ...items.filter((it) => it.saved && it.savedThumbDataUrl)];
    ...
    <main className="layout">
      <section className="panel controls">
        ...
        <div className="actions stack">
          ...
          <button className="secondary" type="button" onClick={handleOpenBatch}>{text.batchConvert}</button>
        </div>
      ...
      {batchOpen ? (
        <div className="batch-modal" role="dialog" aria-modal="true" aria-label={text.batchTitle}>
          <div className="batch-card">
            <div className="batch-header">
              <div>
                <h2>{text.batchTitle}</h2>
                <p className="muted small">{batchItems.length ? text.batchLoaded(batchItems.length) : text.batchEmpty}</p>
              </div>
              <button type="button" onClick={() => setBatchOpen(false)}>{text.close}</button>
            </div>

            <div className="batch-controls">
              <label className="file-fallback secondary" htmlFor="batchInput">{text.batchAddImages}</label>
              <input id="batchInput" type="file" accept="image/*" multiple onChange={handleAddBatchImages} />
              <div className="field">
                <span>{text.brushSize}</span>
                <input type="range" min="1" max="16" value={batchBrushSize} onChange={(e) => setBatchBrushSize(Number(e.target.value))} />
                <div className="pill">{batchBrushSize}×{batchBrushSize} ({batchBrushSize * batchBrushSize}px)</div>
              </div>
              <div className="field">
                <span>{text.colorCount}</span>
                <input type="range" min="2" max="64" value={batchColorCount} onChange={(e) => setBatchColorCount(Number(e.target.value))} />
                <div className="pill">{batchColorCount} {text.colors}</div>
              </div>
              <button type="button" disabled={!batchItems.length} onClick={handleRunBatch}>{text.batchRun}</button>
            </div>

            <div className="batch-gallery">
              {batchItems.map((item) => (
                <div key={item.id} className="batch-thumb">
                  <button type="button" className="remove-thumb" onClick={() => handleRemoveBatchItem(item.id)}>×</button>
                  <img src={item.thumbDataUrl} alt={item.name} />
                  <span>{item.name}</span>
                </div>
              ))}
